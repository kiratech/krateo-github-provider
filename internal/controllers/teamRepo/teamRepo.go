...
func Setup(mgr ctrl.Manager, o controller.Options) error {
	name := managed.ControllerName(teamRepov1alpha1.TeamRepoGroupKind)
...
func (e *external) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	cr, ok := mg.(*teamRepov1alpha1.TeamRepo)
	if !ok {
		return managed.ExternalObservation{}, errors.New(errNotTeamRepo)
	}
	...
	permitted, exists := permissions[spec.Permission]
	if !exists {
	...
	if !permitted {
		...
		e.rec.Eventf(cr, corev1.EventTypeNormal, "NotPermitted", "Team %s/%s misses %s permission to repo %s", spec.Org, spec.TeamSlug, spec.Permission, spec.Repo)
